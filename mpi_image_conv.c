...
int calculate_convolution(uint8_t* imgcurr, uint8_t* imgnext, int startRow, int endRow, int startCol,
		int endCol, int locCols, int imgType, float filter[3][3]);
...
void print_log_to_file(int nProcs, char* imgName, int nrows, int ncols, int maxiters, int conviters,
		double readtime, double runtime, double writetime);
...
/*	int n_edge_cntrmult=-1, n_edge_cntrmin=-1, n_edge_cntrmax=-1, n_edge_stdpart_l=-1, n_edge_stdpart_r=-1;
	...
	for (iter=0; iter < maxIters; iter++){
		...
		while ( ! ( north_rcv_flag && south_rcv_flag && west_rcv_flag && east_rcv_flag &&
			...
			if ( north_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, locRows, locRows, 2, locCols-1, locCols, imgType, filter);
			...
			if ( south_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, 1, 1, 2, locCols-1, locCols, imgType, filter);
				if (South == -1 ){
					for (i=halo;i<imgWidth-halo;i++) {
			...
			/* If yes calc edge convolution */
			if ( west_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, 2, locRows-1, 1, 1, locCols, imgType, filter);
				if (West == -1 ){
			...
			/* If yes calc edge convolution */
			if ( east_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, 2, locRows-1, locCols, locCols, locCols, imgType, filter);
				if (East == -1 ){
					for (i=1 ; i<locRows+1 ; i++) {
						for (k=0; k<halo; k++){
			...
			/* If yes calc edge convolution */
			if ( nwest_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, locRows, locRows, 1, 1, locCols, imgType, filter);
				if (NorthWest == -1 ){
					for (k=0; k<halo; k++){
			...
			/* If yes calc edge convolution */
			if ( neast_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, locRows, locRows, locCols, locCols, locCols, imgType, filter);
				if (NorthEast == -1 ){
					for (k=0; k<halo; k++){
			...
			/* If yes calc edge convolution */
			if ( swest_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, 1, 1, 1, 1, locCols, imgType, filter);
				if (SouthWest == -1 ){
					for (k=0; k<halo; k++){
			...
			/* If yes calc edge convolution */
			if ( seast_rcv_flag ) {
				changed = calculate_convolution(imgcurr, imgnext, 1, 1, locCols, locCols, locCols, imgType, filter);
				if (SouthEast == -1 ){
					for (k=0; k<halo; k++){
	...
	if (procId == 0) {
		...
		print_log_to_file(nProcs, imgName, totalRows, totalCols, maxIters, convIters,
				relapsedTime, elapsedTime, welapsedTime);
	}
...
void print_log_to_file(int nProcs, char* imgName, int nrows, int ncols, int maxiters, int conviters,
	...
	sprintf(mtime, "%02d-%02d-%04d %02d:%02d:%02d", (int)(ptm->tm_mday), (int)(ptm->tm_mon), (int)(ptm->tm_year),
			(int)(ptm->tm_hour), (int)(ptm->tm_min), (int)(ptm->tm_sec));

/*	char* mtime;
	char* delim=malloc(5*sizeof(char));
	delim ="\n;";
	mtime = strtok(asctime(currtime), delim);*/

	char* logline=malloc(300*sizeof(char));
	sprintf(logline,"\n%s\t%s\t%d\t%d\t%d\t%d\t%d\t%3.2f secs\t%3.2f secs\t%3.2f secs",
			mtime, imgName, nrows, ncols, maxiters, conviters,
			nProcs, runtime, readtime, writetime);
...
inline int calculate_convolution(uint8_t* imgcurr, uint8_t* imgnext, int startRow, int endRow, int startCol,
	...
	/* Series of pixels in rows*/
	for ( row = startRow; row <= endRow ; row++){

		/* Calculate for current row series per column pixel */
...
/*
	 Calculate according to image type (grey: 1 bytes/pixel , rgb : 3 bytes/pixel)
	if (imgType == IMG_GREY){
		...
		for ( row = startRow; row <= endRow ; row++){
			...
			for ( col = startCol; col <= endCol ; col++ ) {
				 ...
				 Get previous pixel value
				prevres = imgcurr[imgWidth*row + col];
				int cpx=20;
				if (( row == cpx ) && (col == cpx)){
					printf("pixel %d initial value is %d\n", imgWidth*row+col, prevres);
				}
				...
				for (i=-s; i <= +s; i++){
					for (j=-s; j <= +s; j++){
						 Add each contribution to result
						nextres = nextres + imgcurr[imgWidth*(row-i) + (col-j)] * filter[1+i][1+j];

						if (( row == cpx ) && (col == cpx)){

							printf(" (imgcurr[%d]=%d)*(filter[%d][%d]=%f) = %f -> imgnext[%d]=%f\n",
									imgWidth*(row-i) + (col-j), imgcurr[imgWidth*(row-i) + (col-j)],
									1+i, 1+j, filter[1+i][1+j], imgcurr[imgWidth*(row-i) + (col-j)] * filter[1+i][1+j],
									imgWidth*(row-i) + (col-j), nextres);
						}
					}
				}
				if (( row==cpx) && (col==cpx)){
					printf("pixel %d next value is %d\n", imgWidth*row+col, nextres);
				}

				 Check if pixel has changed
				if ( nextres != prevres ){ imgChanged = 1; }
				 Set pixel new value to next image array
				imgnext[imgWidth*row + col] = nextres;
			}
		}

	} else if (imgType == IMG_RGB) {

		float prevRres, prevGres, prevBres ;
		float nextRres=0.0, nextGres=0.0, nextBres =0.0;
		imgWidth = 3*(locCols + 2);
		...
		for ( row = startRow; row <= endRow ; row++){
			...
			for ( col = startCol; col <= endCol ; col++ ) {
				 ...
				 Get previous pixel value
				prevRres = imgcurr[imgWidth*row + 3*col];
				prevGres = imgcurr[imgWidth*row + 3*col + 1];
				prevBres = imgcurr[imgWidth*row + 3*col + 2];
				...
				for (i=-s; i <= +s; i++){
					for (j=-s; j <= +s; j++){
						 Add each contribution to result
						nextRres += imgcurr[imgWidth*(row-i) + 3*(col-j)] * filter[1+i][1+j];
						nextGres += imgcurr[imgWidth*(row-i) + 3*(col-j) + 1] * filter[1+i][1+j];
						nextBres += imgcurr[imgWidth*(row-i) + 3*(col-j) + 2] * filter[1+i][1+j];
					}
				}
				 Check if pixel has changed
				if ( nextRres != prevRres || nextGres != prevGres || nextBres != prevBres){
					imgChanged = 1;
				}
				 Set pixel new value to next image array
				imgnext[imgWidth*row + 3*col] = nextRres;
				imgnext[imgWidth*row + 3*col+1] = nextGres;
				imgnext[imgWidth*row + 3*col+2] = nextBres;
			}
		}
